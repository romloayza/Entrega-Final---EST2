...
```{r}
prov_vivienda$tasa_agua <- (prov_vivienda$agua_si / prov_vivienda$agua_total) * 3
```

```{r}
prov_vivienda$tasa_elec <- (prov_vivienda$elec_si / prov_vivienda$elec_total) * 3
```
*Tasa por provincia de las viviendas que no tienen internet por cada 4 casas

```{r}
dep_vivienda$tasa_agua <- (dep_vivienda$agua_si / dep_vivienda$agua_total) * 100
```

```{r}
dep_vivienda$tasa_elec <- (dep_vivienda$elec_si / dep_vivienda$elec_total) * 100
```
...
```{r}
prov_inter$tasa_internet <- (prov_inter$internet_si / prov_inter$internet_total) * 3
```

```{r}
prov_inter$tasa_celinter <- (prov_inter$celinter_si / prov_inter$celinter_total) * 3
```
*Tasa por provincia de las viviendas que no tienen internet por cada 4 casas

```{r}
dep_inter$tasa_internet <- (dep_inter$internet_si / dep_inter$internet_total) * 100
```

```{r}
dep_inter$tasa_celinter <- (dep_inter$celinter_si / dep_inter$celinter_total) * 100
```
* Tasa de viviendas que no tienen internet por cada 5 viviendas
